"""
Tests complets du module api_auth :
inscription, connexion, déconnexion, OAuth2 et documentation OpenAPI.
"""
...
# ---------------------------------------------------------------------
class TestApiAuth:
    """Couvre l'ensemble du flux d'authentification de l'API SoftDesk."""
    ...
    def setup_method(self):
        ...
        # Secret utilisé pour l'application OAuth2 (grant_type=password)
    ...
    def _basic_auth_header(self):
        """Construit un header Basic Auth valide pour OAuth2."""
    ...
    def _post_form(self, url, data, **headers):
        """Envoie une requête POST en form-urlencoded."""
    ...
    def test_register_creates_user_successfully(self):
        """Un utilisateur peut s'inscrire correctement."""
    ...
    def test_register_invalid_data_returns_400(self):
        """Une inscription invalide renvoie bien 400."""
        invalid_data = {
            "username": "",
            "email": "not_an_email",
            "password": "",
    ...
    def test_authenticated_user_cannot_register_returns_403(self):
        """Un utilisateur connecté ne peut pas s’inscrire à nouveau."""
    ...
    def test_openapi_register_schema_includes_expected_responses(self):
        ...
        assert (
            register_path
        ), "L’endpoint /api-auth/register/ n’est pas présent."

        post_responses = (
            paths[register_path].get("post", {}).get("responses", {})
        )
        for code in ("201", "400", "403"):
            assert (
                code in post_responses
            ), f"Code {code} manquant dans la doc OpenAPI."

    # -----------------------------------------------------------------
    # TESTS DE CONNEXION / DÉCONNEXION
    # -----------------------------------------------------------------
    def test_login_redirects_authenticated_user(self, client):
        """Un utilisateur connecté est redirigé vers /api/."""
    ...
    def test_login_view_returns_html_for_authenticated_user(self):
        """La vue de connexion retourne bien un formulaire HTML."""
        self.client.force_authenticate(user=self.user)
    ...
    # -----------------------------------------------------------------
    # TESTS DE PAGE D’ACCUEIL
    # -----------------------------------------------------------------
    def test_api_auth_homepage_renders(self, client):
        """Vérifie que la page d’accueil API Auth s’affiche."""
    ...
    def test_oauth2_token_obtain_and_use(self):
        """Obtention, usage et rafraîchissement du token OAuth2."""
        token_url = "/o/token/"
        payload = {
            "grant_type": "password",
            "username": "existing",
        ...
        # Vérifie qu’un token valide peut accéder à une route protégée
    ...
    def test_oauth2_invalid_credentials(self):
        """Une erreur est levée avec un mauvais mot de passe."""
        token_url = "/o/token/"
        payload = {
            "grant_type": "password",
            "username": "existing",
